...
const layout = {
  ...
  A: [200, 500],  // sol aşağı
  M: [350, 430],  // sol orta aşağı
  E: [500, 650],  // bottom (9)
  N: [650, 430],  // N (KÜ5) biraz yuxarı bottom
  G: [500, 350],  // mərkəz (özünüreallaşdırma)
  K: [500, 500],  // sağ orta yuxarı
  V: [800, 500],  // sağ aşağı
...
function drawSkeleton() {
  ...
  const lineSets = [
    [p.B, p.L, p.V, p.A, p.I, p.B, p.J, p.G, p.N, p.V],    // ulduzun dairəvi bağlantıları                // alt-orta xəttlər
    [p.D, p.J, p.Z, p.E, p.D],                        // sol yuxarı üçbucaq
    [p.A, p.M, p.G, p.L, p.Z],                        // orta üçbucaq
    [p.D, p.I, p.G, p.K, p.E]                         // alt üçbucaq
  ];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dayEl && !dayEl.value) dayEl.value = "";
